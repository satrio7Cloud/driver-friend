...
func (r *AuthRoutes) RegisterRoutes(router *gin.RouterGroup) {
	auth := router.Group("/auth")
	{
		...
		auth.POST("/driver/request-otp",
			middleware.OTPRateLimiterByPhone(),
			r.authController.RequestDriverOTP,
		)
		auth.POST("driver/verify-otp",
			middleware.OTPRateLimiterByPhone(),
			r.authController.VerifyDriverOTP,
		)
